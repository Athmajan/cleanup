...
from envs.map_env import MapEnv, MAP_ACTIONS
...
# Custom colour dictionary -Fixed incorrect colour details

CLEANUP_COLORS = {'C': [100, 255, 255], # CLEANING BEAM CYAN   
                  'S': [99, 156, 194],  # STREAM  BLUE   
                  'H': [113, 75, 24],  # WASTE  BROWN     
                  'R': [99, 156, 194]}  # RIVER BLUE     
...
class CleanupEnv(MapEnv):
    ...
    def setup_agents(self):
        ...
        for i in range(self.num_agents):
            ...
            agent = CleanupOnlyAgent(agent_id, spawn_point, rotation, map_with_agents)
...
class CleanupAgent(Agent):
    ...
    CLEANUP_ACTIONS.update({8: FIRE_ACTION,  # Fire a penalty beam
                            7: CLEAN_ACTION})  # Fire a cleaning beam
...
class CleanupOnlyAgent(Agent):

    CLEANUP_ACTIONS = BASE_ACTIONS.copy()
    CLEANUP_ACTIONS.update({7: CLEAN_ACTION})  # Fire a cleaning beam
